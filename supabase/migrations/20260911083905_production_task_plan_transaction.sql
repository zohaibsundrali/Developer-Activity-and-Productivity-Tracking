...
declare project_row public.projects%rowtype; item jsonb; member_id uuid; saved jsonb;
begin
  ...
  if project_row.task_plan_status is distinct from 'pending' then
    ...
    perform 1 from public.developer_tasks where project_id=p_project and developer_id=p_developer
      and organization_id=p_org and coalesce(status,'pending')='pending' for update;
    ...
    select p_org,p_project,p_developer,btrim(value->>'task_title'),coalesce(value->>'task_description',''),
      (ordinality-1)::int,(value->>'start_date')::date,(value->>'end_date')::date,'pending',now(),now()
