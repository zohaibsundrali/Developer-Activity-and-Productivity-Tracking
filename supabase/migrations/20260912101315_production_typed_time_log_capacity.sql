-- Apply after 20260912100844_production_transactional_timesheet_review.sql.
-- Log ownership is now a typed profile identity. Do not merge colliding admin
-- and developer IDs or guess the owner type of unresolved legacy records.
-- Leave, project allocation, and developer-task backlog semantics are retained.
begin;
...
logged as (
  select l.organization_id,
    ...
    from public.task_time_logs l
   where l.seconds is not null and l.user_type in ('admin','developer') and l.organization_id=p_org
     and public.timesheet_week_of(l.started_at)=p_week
   group by l.organization_id, l.developer_id, l.user_type, public.timesheet_week_of(l.started_at)
),
leave_days as (
...
with weeks as (
  ...
  select l.organization_id,
         ...
         public.timesheet_week_of(l.started_at) as week_start
    from public.task_time_logs l
   where l.seconds is not null and l.user_type in ('admin','developer')
   group by l.organization_id, l.developer_id, l.user_type, public.timesheet_week_of(l.started_at)
  union
