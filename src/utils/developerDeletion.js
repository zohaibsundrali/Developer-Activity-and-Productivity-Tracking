 ...
 * exist, and it is reversible. This removes the projects, the tasks, the
 * submissions and the activity log, and it is not. Almost every real case —
 * somebody left the company — wants the first one.
 *
...
export async function deleteDeveloperAccount({ developer, actor }) {
  ...
  // Ownership pre-check, where the row records who added them. The route
  // repeats it against the caller's verified token — this only turns a 403
  // into a sentence.
  if (actor) {
  ...
  try {
    ...
    showPre(
      ...
      `Developer deleted successfully!\n\n` +
        ...
        `• Projects:    ${summary?.relatedDataDeleted?.projects ?? 0}\n` +
