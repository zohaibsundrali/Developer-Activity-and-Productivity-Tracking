...
export default function MobileFieldHistory(){
 ...
 const permissionBinding=JSON.stringify(['attendance.view_own','attendance.view_all','monitoring.view','attendance.manage'].map(key=>allowed(key)));
 const ticket=useRef(0),pending=useRef(false),abort=useRef(null);const binding=`${authStatus}:${identity}:${permissionBinding}:${from}:${to}:${scope}:${refresh}`,live=useRef(binding);live.current=binding;
 const state=result?.binding===binding&&(scope!=='all'||(allowed('attendance.view_all')&&allowed('monitoring.view')))?result.data:null;
 const current=(captured,generation)=>live.current===captured&&ticket.current===generation&&reportIdentity(getOrgContext())===identity&&allowed('attendance.view_own')&&(scope!=='all'||(allowed('attendance.view_all')&&allowed('monitoring.view')));
