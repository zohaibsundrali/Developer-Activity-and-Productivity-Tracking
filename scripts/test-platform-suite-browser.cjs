...
(async () => {
  ...
  try {
    ...
    await page.route('**/api/platform/**', async route => {
      ...
      else if (path === 'analytics') result = { currencies: [{ currency: 'usd', mrr_cents: 15000, arr_cents: 180000, paid_cents: 30000, refunded_cents: 1000, net_collected_cents: 29000, outstanding_cents: 5000 }], churn: { canceled: 1, starting_subscriptions: 20, rate: 5 }, notes: ['MRR is an estimate based on the current plan catalog.'] };
