...
set -euo pipefail
cd "$(dirname "$0")/.."
sync_container="tracking-github-sync-test-$$"
...
docker exec "$sync_container" psql -U postgres -v ON_ERROR_STOP=1 -f /workspace/database/tests/github_issue_task_sync.sql

python3 scripts/test-github-sync-concurrency.py "$sync_container"
