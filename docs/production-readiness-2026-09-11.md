...
## Parallel follow-up: submission transactions and contributor access

The proof submission endpoint previously performed proof, task state, activity and notification writes separately. A failed proof insert could still clear a rejected task's verdict and report success. Migration `20260911090852_production_submission_transaction.sql` now commits these records together, verifies the actual stored object and its metadata, repeats typed membership and submission permissions under the organization/task locks, and supports retry with the same proof without duplicate records. The browser reuses its successfully uploaded file during retries in the same mounted modal. File upload itself remains separate from the database transaction; persistent abandoned-upload recovery and immutable proof storage still require further review.

The submission history endpoint now requires either an effective all-task/reviewer permission or an effective own-task permission with a verified Developer profile. An Admin profile with a coincident UUID cannot use the own-assignment fallback. Generic failures no longer expose raw exception details. Submission URLs are constructed from configured storage origin and verified object paths rather than trusting the supplied URL.

Parallel review also corrected rejected-plan resubmission duplication: persisted task IDs survive browser payload construction, and the transaction retains started/linked work instead of inserting duplicate copies. Review transactions reject proof belonging to a different assignee or project after task reassignment. The contributor project page gates mounting and separate edit/start/submit actions by effective permissions; database edits require returned affected rows before updating browser state. Dashboard direct section URLs respect overrides, and My Projects includes typed project staffing as well as legacy Developer assignments, with explicit loading/retry/error states.

These changes amend two still-local pending transaction migrations and add a fifteenth pending migration. They have not been applied to the connected Supabase project. Direct task-table permissions, related-record policies, notification insertion/identity rules, and external production journeys remain under audit; this is not a certification of complete production readiness.

Independent submission review confirmed remaining bypasses in the historical authenticated `task_submissions` INSERT/UPDATE/DELETE policies and mutable uploaded proof objects. These can bypass the new service transaction and are next audit work; the new RPC alone does not establish end-to-end proof integrity. Its retry cache also does not survive modal unmount/reload, so an ambiguous committed submission can require refreshing rather than uploading new proof.

Combined validation for this batch: **106 web test files / 3,261 tests passed**, including typed own-submission reads, effective dashboard access and confirmed-row task edits. The complete isolated SQL suite passed with task-plan duplicate-ID protection, stale-proof review rejection, submission failure rollback, permission denial, service-only execution and same-proof retry coverage. No real email, payment or production business data was written.

Further read-only policy review found that `developer_tasks` still grants organization staff broad direct access regardless of task overrides. A compatible repair must retain the existing contributor plan editor: effective own-update permission plus an editable assigned project's plan permits title/description/date edits and explicit deletion, including started rows currently shown by that page. The My Work, QA bug and client-published-task exceptions need separate mapping before changing those policies. Historical comment policies also allow client comment UPDATE/DELETE despite the portal's append-only design, and lack typed author/parent integrity; attachments need parent-task and uploader/path checks. These findings are not yet fixed by the submission transaction batch.

Final production build for the combined batch passed with existing lint warnings. The new submission migration must be installed before deploying this route change. Changes remain local; GitHub publishing and live migrations are still unavailable in the current connection state.

## Direct proof-write authority

Migration `20260911094725_production_submission_write_authority.sql` closes the historical direct-write bypass on `task_submissions`. Restrictive authenticated INSERT/UPDATE/DELETE policies now require application writes to use the already-authorized service transactions. This remains effective alongside permissive legacy policies and does not alter SELECT access. Source inventory found no remaining direct browser submission writer; developer deletion and reset maintenance use service clients.

Verification: the complete isolated SQL suite passed with this migration installed before the real review/submission transaction fixtures. A standalone adversarial fixture also passed: Owner, Admin, Manager and Developer direct inserts are refused, updates/deletes affect no rows, existing reads remain available, service-role mutations work, and an authorized parent deletion still cascades. There were no JavaScript changes in this follow-up; the last web evidence remains 106 files / 3,261 passing tests and a passing production build. Sixteen pending production migrations now exist. Storage object mutability, typed notification privacy and task/comment access remain separate ongoing work.

## Comment attribution and immutable proof objects

Migration `20260911095524_production_task_comment_integrity.sql` makes comment access inherit the actual parent task's RLS and verifies active typed membership. Inserts require the authenticated author/profile type and derive the displayed author name from that profile. Task, organization, author and creation identity cannot be rewritten. Staff moderation is preserved; clients retain permitted public-thread insertion and reads, but cannot update/delete comments. Parent task restrictions remain a separate migration under implementation.

Migration `20260911095840_production_proof_object_immutability.sql` prevents authenticated overwrite, move and deletion of `submissions/` proof objects in the shared task-submissions bucket, including files not yet linked to a submission. This avoids a reference-check race during submission. Ordinary `pm/` attachments retain their existing policies. Proof upload now uses insert rather than upsert, and the existing retry cache reuses its stored object. Trusted retention/abandoned-upload cleanup remains a service responsibility; this change does not verify public bucket download privacy or implement an abandoned-upload cleanup job.

The complete isolated PostgreSQL suite passed, including client append-only behavior, forged display names/typed authors, immutable comment identity, inherited parent visibility, proof read/new upload, overwrite/move/delete denial, ordinary attachment updates/deletion, and trusted proof cleanup. An initial test failure came from a shared test role lacking Supabase's service-role BYPASSRLS attribute; the fixture was corrected and the full suite rerun successfully. Storage tests use synthetic metadata, not real production Storage API calls. Notification identity and full task authorization changes remain uncommitted work under separate verification.
