...
Verification completed in an isolated PostgreSQL 16 fixture (`database/tests/client_support_transactions.sql`): failed first-message insertion rolls back the new thread; failed thread activity update rolls back the reply; revoked/wrong-type members, wrong tenants, other clients and browser RPC callers are rejected. No production data was mutated during this verification. API tests are in `tests/clientSupportTransactions.test.js` within the pending activation commit `d8b6c2b` (local branch `qa/support-transactions-pending-sql`; patch `/tmp/verisade-support-activation.patch`).
