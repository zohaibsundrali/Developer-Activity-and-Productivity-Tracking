...
- Complete `scripts/test-audit-policies.sh` PostgreSQL policy/concurrency regression passed, including all eight new migration fixtures and three competing proposal-decision scenarios. Focused tests passed for each new migration, including rollback on notification failure, permission overrides, typed identity collisions, scoped writes and replay behavior. Automation mutation and notification triggers were also executed together. An additional proposal fixture ran real typed-manager roster and project-assignment triggers together: acceptance/retry retained one manager notice; a later separate assignment produced one assignee notice, and all three competing decision scenarios passed again.
