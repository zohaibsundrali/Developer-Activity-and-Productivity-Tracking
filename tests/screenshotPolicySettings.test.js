...
const { default: Settings, validScreenshotInterval } = await import('@/components/shared/ScreenshotPolicySettings');
const policy = { organization_id: 'org-a', enabled: true, interval_seconds: 60, can_manage: true };
...
const saveButton = tree => find(tree, node => node.type === 'button' && node.props.children === 'Save screenshot policy');
...
it.each(['', '59', '3601', '60.5', 'Infinity', '1e2', '-60'])('rejects interval %s', value => expect(validScreenshotInterval(value)).toBe(false));
it.each(['60', '3600', '120'])('accepts interval %s', value => expect(validScreenshotInterval(value)).toBe(true));
it('loads authoritative policy and only enables save after an edit', async () => {
  let tree = await mount(); expect(h.rpc).toHaveBeenCalledWith('get_screenshot_policy');
  expect(saveButton(tree).props.disabled).toBe(true);
  control(tree, 'Screenshot interval in seconds').props.onChange({ target: { value: '120' } });
  tree = render(); expect(saveButton(tree).props.disabled).toBe(false);
  h.rpc.mockResolvedValueOnce({ data: { ...policy, interval_seconds: 120 } });
  await saveButton(tree).props.onClick();
  expect(h.rpc).toHaveBeenLastCalledWith('set_screenshot_policy', { p_enabled: true, p_interval_seconds: 120 });
...
it.each([{ readOnly: true }, { serverDenied: true }])('does not expose edit controls to readonly user %j', async options => {
  ...
  const tree = await mount(options); expect(control(tree, 'Enable screenshots')).toBeNull(); expect(saveButton(tree)).toBeNull();
});
...
it('does not send stale organization edits after account context changes', async () => {
  let tree = await mount(); control(tree, 'Enable screenshots').props.onChange({ target: { checked: false } });
...
it('keeps unsaved changes and reports a save denial', async () => {
  ...
  tree = render(); expect(control(tree, 'Enable screenshots').props.checked).toBe(false);
