...
beforeEach(() => {
  ...
  client = {
    ...
    rpc: vi.fn(async (_name, args) => ({ data: { success: true, capture_id: args.p_capture_id, storage_path: args.p_metadata.storage_path }, error: null })),
  };
...
describe('device screenshot durable receipt', () => {
  it('derives required metadata from verified identity and PNG bytes, uploads immutably', async () => {
    ...
    const metadata = client.rpc.mock.calls[0][1].p_metadata;
    ...
    expect(client.rpc.mock.calls[0][0]).toBe('finalize_screenshot_capture');
  ...
  it('rejects conflicting metadata before touching storage', async () => {
    await POST(request());
    receipt = { storage_path: path, capture_metadata: client.rpc.mock.calls[0][1].p_metadata };
  ...
  it('reuses the committed timestamp when an older client omitted it', async () => {
    ...
    expect(client.rpc.mock.calls[1][1].p_metadata).toEqual(receipt.capture_metadata);
  });
