...
vi.mock('@/utils/serverAuth', () => ({
  ...
  serviceClient: () => ({
    ...
    from: table => { s.calls.push(['service', table]); const q = {
      select: () => q, eq: () => q, in: () => q,
      maybeSingle: async () => ({ data: table === 'projects' ? { created_by: 'shared', added_by: 'shared' } : { name: 'Developer', full_name: 'Admin' } }),
...
beforeEach(() => { s.auth = { orgId: 'org', token: 'jwt', role: 'manager', userType: 'developer', overrides: {} }; s.task = { id, project_id: 'project' }; s.eligible = { admin: false, developer: true }; s.calls = []; s.error = null; });
