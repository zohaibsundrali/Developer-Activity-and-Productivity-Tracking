...
it.each([['23P01',409],['55000',409],['PT409',409], ['40001',409],['42501',403],['22023',400],['XX000',503]])('maps %s without database internals',async(code,status)=>{h.result={error:{code,message:'private-database-details'}};const response=await post();expect(response.status).toBe(status);expect(await response.text()).not.toContain('private-database');});
