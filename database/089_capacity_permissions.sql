-- ============================================================================
--  089 - role_permissions: the capacity keys
--
--  WHAT CHANGED
--
--  Migration 088 made capacity a measurement rather than a convention. Two
--  keys:
--
--    capacity.allocate       owner, admin, manager
--    employment.set_hours    owner, admin, hr
--
--  TWO DIFFERENT ACTS, AND NEITHER IS capacity.view. Setting a project
--  allocation is a staffing decision, and its role list is exactly the one
--  071's project_members write policy already uses -- so the key and the RLS
--  cannot come to disagree about who may staff a project.
--
--  Setting somebody's contracted weekly hours is an employment fact about a
--  PERSON, not about a project, so it belongs to HR. A manager who could
--  quietly raise a report's weekly hours could make their own plan come out
--  right, which is the one way this feature could be used to lie.
--
...
-- PART 1 -- every row the catalogue defines. 429 rows over 95 keys.
...
-- ---------------------------------------------------------------------
--  Expect 429 rows over 95 distinct keys.
