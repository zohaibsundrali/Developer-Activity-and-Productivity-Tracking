-- ============================================================================
--  082 - role_permissions: the quality keys
--
--  WHAT CHANGED
--
--  Migration 081 added test cases, test runs and the defect link. Four keys:
--
--    test_case.view    test_run.execute    owner, admin, manager, team_lead,
--                                          qa, developer, designer, devops
--    test_case.manage  test_run.manage     owner, admin, manager, team_lead, qa
--
--  READING IS WIDER THAN WRITING BY THE THREE CONTRIBUTOR ROLES, and the
--  asymmetry is the whole design. A developer needs to see what will be checked
--  before calling something finished, and needs to record a result during a
--  run -- testing is not something QA does alone. Writing the CASES is
--  narrower: what to check is a QA decision, and a developer editing the test
--  that judges their own work is the one shape this module refuses.
--
--  hr, finance and employee hold none of them: none of the three is on a
--  project. client holds none: a test plan is internal.
--
...
-- PART 1 -- every row the catalogue defines. 382 rows over 84 keys.
...
-- ---------------------------------------------------------------------
--  Expect 382 rows over 84 distinct keys.
