do $$ declare org uuid:='00000000-0000-0000-0000-000000000096'; reviewer uuid:=gen_random_uuid();
  developer uuid:=gen_random_uuid(); project uuid; task uuid; submission uuid; result jsonb;
begin
  ...
  insert into task_submissions(organization_id,task_id,developer_id,submitted_at)
    values(org,task,developer,'2026-09-11 23:59:59.999+00') returning id into submission;
  ...
  insert into task_submissions(organization_id,task_id,developer_id,submitted_at)
    values(org,task,developer,'2026-09-12 00:00:00+00') returning id into submission;
  ...
  insert into task_submissions(organization_id,task_id,developer_id,submitted_at)
    values(org,task,developer,'2026-09-11 12:00:00+00') returning id into submission;
