-- ============================================================================
--  080 - role_permissions: the invoicing keys
--
--  WHAT CHANGED
--
--  Migration 079 added invoicing from approved hours and project P&L. Three
--  keys came with it, and one existing key was widened by one role:
--
--    invoice.view   invoice.manage   pnl.view      owner, admin, finance
--    timesheet.view_all  + finance
--
--  THE WIDENING IS THE PROMISE 077 MADE. That migration introduced
--  timesheet.view_all without finance and said why: billable hours feed
--  invoicing and finance would need them, with an invoice in front of it rather
--  than as a widening smuggled in early. 079 is that invoice. Approving a week
--  is still delivery's call, so timesheet.approve is untouched.
--
--  pnl.view IS NOT GIVEN TO manager, and that is the one exclusion worth
--  arguing about. P&L is revenue minus cost, and cost is hours times
--  employee_profiles.cost_rate -- what a person is paid, spread over an hour.
--
...
-- PART 1 -- every row the catalogue defines. 362 rows over 80 keys.
...
-- ---------------------------------------------------------------------
--  Expect 362 rows over 80 distinct keys.
