-- ============================================================================
--  096 - role_permissions: the staff test surface
--
--  WHAT CHANGED
--
--  095 widened two keys and gave one act a name of its own:
--
--    test_case.view    + developer, designer, devops, employee
--    test_run.execute  + developer, designer, devops, employee
--    bug.raise         NEW -- owner, admin, manager, team_lead, qa
--
--  `bug.raise` holds precisely the roles `test_run.execute` held before today,
--  so nobody gains or loses the ability to file a defect. It exists so that
--  widening execute -- which is about running a test -- did not quietly widen
--  writing a `developer_tasks` row, which is task creation and is SUPERVISORS
--  everywhere else in this product.
--
--  NOTHING NARROWED, so an upsert leaves no stale row behind. Said explicitly
--  because it is the condition under which upsert-only is complete; a future
--  sync that removes a role from a key needs a delete as well.
--
...
--
--  103 keys, from 102. 465 rows, from 452.
--
--  RUN AFTER 095.
-- ============================================================================

-- PART 1 -- every row the catalogue defines. 465 rows over 103 keys.
insert into public.role_permissions (role, resource, action, allowed) values
  ...
  ('designer', 'test_case', 'view', true),
  ('designer', 'test_run', 'execute', true),
  ...
  ('developer', 'test_case', 'view', true),
  ('developer', 'test_run', 'execute', true),
  ...
  ('devops', 'test_case', 'view', true),
  ('devops', 'test_run', 'execute', true),
  ...
  ('employee', 'test_case', 'view', true),
  ('employee', 'test_run', 'execute', true),
...
-- ---------------------------------------------------------------------
--  Expect 465 rows over 103 keys.
select count(*) as total_rows,
       count(distinct (resource || '.' || action)) as distinct_keys,
       count(distinct role) as roles
  from public.role_permissions;

--  The three keys this migration is about, by role.
select resource || '.' || action as permission_key,
       string_agg(role, ', ' order by role) as roles
  from public.role_permissions
 where allowed
   and (resource, action) in (('test_case','view'), ('test_run','execute'), ('bug','raise'))
 group by 1
 order by 1;
...
-- ---------------------------------------------------------------------
--  Rows in the mirror naming a key the catalogue no longer defines. Expect
--  none; a row here is a key that was renamed without a sync.
select rp.role, rp.resource, rp.action, rp.allowed
