-- ============================================================================
--  086 - role_permissions: the recruitment keys
--
--  WHAT CHANGED
--
--  Migration 085 added job openings, candidates and the hiring pipeline.
--  Four keys:
--
--    job.view                     owner, admin, hr, manager, team_lead
--    job.manage / candidate.view / candidate.manage    owner, admin, hr
--
--  AN OPENING IS NOT PII; A CANDIDATE IS, and that asymmetry is the reason
--  there are four keys rather than two. Knowing the company is hiring a QA
--  engineer is ordinary workplace information a manager planning next quarter
--  needs. Knowing that a named person applied, and what an interviewer thought
--  of them, is information about somebody who is not in this organization and
--  never agreed to be discussed in it.
--
--  THE HIRING MANAGER IS NOT A KEY. Whoever is named hiring_manager_id on an
--  opening reads that opening's candidates and no others, granted by the RLS
--  policy on the row -- the same shape as a reviewer reading what they wrote
--  in 083. A role cannot express "this opening but not that one".
--
...
-- PART 1 -- every row the catalogue defines. 423 rows over 93 keys.
...
-- ---------------------------------------------------------------------
--  Expect 423 rows over 93 distinct keys.
