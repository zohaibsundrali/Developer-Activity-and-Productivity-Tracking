-- ============================================================================
--  076 - role_permissions: the attendance and leave keys
--
--  WHAT CHANGED
--
--  Migration 075 added the attendance and leave module. Nine keys came with it:
--
--    attendance.view_own   attendance.log_own    leave.view_own
--    leave.request_own     attendance.view_all   attendance.manage
--    leave.view_all        leave.approve         leave.manage_types
--
--  The first four are STAFF -- every role except client, because every role
--  except client has a working day and takes holidays. The oversight four are
--  owner/admin/hr/manager, and `attendance.manage` and `leave.manage_types`
--  are owner/admin/hr: a manager may READ their team's attendance and may not
--  rewrite it, because a record somebody's own manager can edit is not a
--  record.
--
--  WHY A FULL RE-SYNC AGAIN, for the reason 074 gives: role_permissions is a
--  MIRROR. Nothing reads it at runtime -- the catalogue is the source of truth
--  and resolvePermission never queries the database -- so this table exists to
--  make the model inspectable in SQL, and a mirror that is partly updated is
--  worse than one that is plainly stale. Writing every row every time is how it
--  cannot drift by a few rows the way 074 found 072 had.
--
--  NOTHING IS DELETED. Every statement is an upsert. A key removed from the
--  catalogue would leave its row behind, which is what PART 3 is for: it lists
--  what the catalogue currently defines so an orphan is visible rather than
--  assumed absent.
--
--  RUN AFTER 075.
-- ============================================================================

-- PART 1 -- every row the catalogue defines. 333 rows over 74 keys.
...
-- ---------------------------------------------------------------------
--  Expect 333 rows over 74 distinct keys.
